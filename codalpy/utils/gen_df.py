...
from codalpy.utils.utils import translate, fiscal_month, pascal_to_snake_case, normalize_fs_item
from codalpy.utils import cols, dicts


def _cells(item: FinancialStatement, fs: Literal["BalanceSheet", "IncomeStatement"]) -> list[Cell] | None:
...
def clean_df(
        records: list[tuple[Letter, FinancialStatement]], category: Literal["production"],
        ...
        if cells is not None:
            ...
            df = df.with_columns(
                pl.struct(["item"]).map_elements(
                    lambda x: translate(
                        x["item"], dicts.dicts[snake_fs][category]
                    ),
                    return_dtype=pl.String,
                )
            )
            df = df.filter(pl.col("item").is_in(dicts.dicts[snake_fs][category].values())).with_columns([
            ...
            df = df.with_columns(
                [
                    ...
                    pl.lit(data.year_end_to_date).alias(
                        "fiscal_year_ending_date"
                    ),
                    pl.lit(
                        fiscal_month(
                            data.year_end_to_date, data.period_end_to_date
                        )
                    ).alias("fiscal_month"),
